...
using SimpleEmployeeManagement.Migrations.SeedData;
...
namespace SimpleEmployeeManagement.Tests
{
    public abstract class SimpleEmployeeManagementTestBase : AbpIntegratedTestBase<SimpleEmployeeManagementTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected SimpleEmployeeManagementTestBase()
        {
        ...
        protected void UsingDbContext(Action<SimpleEmployeeManagementDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SimpleEmployeeManagementDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SimpleEmployeeManagementDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SimpleEmployeeManagementDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SimpleEmployeeManagementDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SimpleEmployeeManagementDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SimpleEmployeeManagementDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SimpleEmployeeManagementDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SimpleEmployeeManagementDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SimpleEmployeeManagementDbContext, Task<T>> func)
        {
            T result;
